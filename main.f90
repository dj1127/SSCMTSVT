...
!                                        innerInt3(iAero,jAero) = res
    ...
    ! relative scaling of states (for each rotor)
    do i = 1,2
        const%YSCALE(i,1) = 1*(10.**(-3))
    end do
    do i = 3,4
        const%YSCALE(i,1) = 1*(10.**(-4))
    end do
    do i = 5,10
        const%YSCALE(i,1) = 1
    end do
    ...
    ! number of states 
    const%NSTATES = size(const%DELXLIN)
    ! number of control inputs 
    const%NCTRLS = size(const%DELCLIN)
    ! number of outputs 
    const%NOUT = 5*const%nRot
    ! trim variables and trim targets 

    allocate(const%TRIMVARS(14))
    allocate(const%TRIMTARG(14))

    NSTATES = const%NSTATES

    if (trimMethod ==1) then
        ! trim variables
        const%TRIMVARS = (/ (I, I = 1, NSTATES) /)
        ! trim targets
        const%TRIMTARG = (/ (I, I = 1, NSTATES) /)
    else if (trimMethod ==2) then
        ! trim variables
        const%TRIMVARS = (/ (I, I = 1, NSTATES) /)
        ! add collective pitch to trim variables 
        do i = 1,const%nRot
            const%TRIMVARS=(/const%TRIMVARS, const%NSTATES+1+(i-1)*const%NCTRLS/const%nRot/)
        end do
        ! trim targets
        const%TRIMTARG=(/ (I, I = 1, NSTATES) /)
        do i = 1,const%nRot
        ...
        const%TRIMVARS = (/ (I, I = 1, NSTATES) /)
        do i = 1,const%nRot
            const%TRIMVARS=(/const%TRIMVARS, const%NSTATES+4+(i-1)*const%NCTRLS/const%nRot/)
        end do
        ! trim targets
        const%TRIMTARG = (/ (I, I = 1, NSTATES) /)
        do i = 1,const%nRot
